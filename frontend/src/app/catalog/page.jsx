...
function CatalogContent() {
  ...
  const handleAddToOrder = async (item) => {
    ...
    try {
      if (!draft.poId) {
        // Creating a new DRAFT PO — invalidate dashboard so it shows updated count
        ...
        draft.setDraft(po);
        const line = await procurementApi.addLineItem(po.id, {
        ...
        const line = await procurementApi.addLineItem(draft.poId, {
          catalogItemId: item.id,
          quantity: 1,
        });
        draft.addItem(line);
  ...
  const isInDraft = (itemId) =>
    draft.items.some((i) => i.catalog_item_id === itemId);
  ...
  return (
    <div>
      <div className="mb-4 sm:mb-6">
        <h1 className="text-xl sm:text-2xl font-bold">Equipment Catalog</h1>
        <p className="text-steel-500 mt-1 text-sm sm:text-base">
          Browse and search refinery equipment for purchase orders
        </p>
      </div>
        ...
        <div className="mb-4 p-3 sm:p-4 bg-red-50 border border-red-200 rounded-xl flex items-start gap-3">
          ...
          <div className="flex-1">
            ...
            <div className="text-xs sm:text-sm text-red-700 mt-1">
              Cannot add &ldquo;{supplierError.itemName}&rdquo; from{" "}
              <strong>{supplierError.itemSupplier}</strong> &ndash; your current
              draft is locked to <strong>{supplierError.poSupplier}</strong>.
              All items in a PO must come from the same supplier.
            </div>
          </div>
          <button
            onClick={() => setSupplierError(null)}
      ...
      <div className="card p-3 sm:p-4 mb-4 sm:mb-6">
        <div className="flex flex-col gap-3">
          ...
          >
            <select
              ...
              className="input-field w-full sm:w-auto min-w-[140px] text-sm"
            >
            ...
            <select
              ...
              className="input-field w-full sm:w-auto min-w-[120px] text-sm"
            >
            ...
            <select
              ...
              className="input-field w-full sm:w-auto min-w-[170px] text-sm"
            >
              {sortOptions.map((o) => (
                <option
        ...
        <div className="mt-3 flex flex-col sm:flex-row sm:items-center justify-between text-xs sm:text-sm text-steel-500 gap-1">
          <span>
            {pagination.total} item{pagination.total !== 1 ? "s" : ""} found
          </span>
          {draft.poId && (
            <span className="text-brand-600 font-medium">
              Draft: {draft.supplierName} ({draft.items.length} item
      ...
      {!isLoading && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 mb-6">
          {items.map((item) => (
            <div
              key={item.id}
              className="card p-4 sm:p-5 flex flex-col hover:shadow-md transition-shadow"
            >
              <div className="flex items-start justify-between mb-2">
                <span className="font-mono text-xs text-steel-400 bg-steel-50 px-2 py-0.5 rounded">
                  {item.id}
                </span>
                <span
                  className={`badge text-xs ${item.in_stock ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700"}`}
                >
                  {item.in_stock ? "In Stock" : `Lead ${item.lead_time_days}d`}
                </span>
              </div>
              <h3 className="font-semibold text-sm text-steel-900 mb-1 line-clamp-2">
                {item.name}
              </h3>
              <div className="text-xs text-steel-500 mb-3 truncate">
                {item.supplier} · {item.manufacturer} · {item.model}
              </div>
              <div className="flex-1">
                <div className="text-xs text-steel-400 mb-1.5">
                  Specifications
                </div>
